...
extern int pixelMessage(char* imageArray, char character, int arraysize);                   // test this 
extern int embed(char* image_data, int data_length, char* message, char** encoded);         // test this
...
{	
	...
	if (source!=NULL && arrsize>0)
	{
        ...
        memcpy(dest, source, arrsize*sizeof(char));
	}
    internal = pixelMessage(dest, c, arrsize);
	if (internal==resultf)
	{
